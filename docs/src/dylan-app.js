...
register$1({
  ...
  info: {
    ...
    started: false,
    moves: [],
    /** @this {GestureRecognizer} */addMove: function (move) {
...
registerEffect('blend-background', {
  /** @this Polymer.AppLayout.ElementWithBackground */setUp: function setUp() {
    var fx = {};
  ...
  /** @this Polymer.AppLayout.ElementWithBackground */run: function run(p, y) {
    var fx = this._fxBlendBackground;
    fx.backgroundFrontLayer.style.opacity = 1 - p;
    fx.backgroundRearLayer.style.opacity = p;
  },
  /** @this Polymer.AppLayout.ElementWithBackground */tearDown: function tearDown() {
...
registerEffect('fade-background', {
  /** @this Polymer.AppLayout.ElementWithBackground */setUp: function setUp(config) {
    var fx = {};
...
registerEffect('resize-title', {
  ...
  /** @this PolymerElement */run: function run(p, y) {
...
Polymer$1({
  ...
  attached: function () {
    ...
    this.listen( /** @type {!HTMLBodyElement} */document.body, 'click', '_globalOnClick'); // Give a 200ms grace period to make initial redirects without any
...
var appRouteConverterBehavior = {
  AppRouteConverterBehavior: AppRouteConverterBehavior
}; /**
   @license
   Copyright (c) 2016 The Polymer Project Authors. All rights reserved.
   This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
...
var IS_V2 = flush$1 != null;
var ANIMATION_FRAME = IS_V2 ? animationFrame : 0;
var IDLE_TIME = IS_V2 ? idlePeriod : 1;
var MICRO_TASK = IS_V2 ? microTask : 2; /* Polymer.OptionalMutableDataBehavior is only available with Polymer 2.0. */

if (!OptionalMutableDataBehavior) {
...
`,
   ...
   */_itemsChanged: function (change) {
    if (change.path === 'items') {
      ...
      this._virtualCount = this.items ? this.items.length : 0; // Only blur activeElement if it is a descendant of the list (#505, #507).

      var activeElement = this._getActiveElement();

      if (this.contains(activeElement)) activeElement.blur(); // Render only if the affected index is rendered.
   ...
   */scrollToIndex: function (idx) {
    ...
    flush$1 ? flush$1() : flush$1(); // Items should have been rendered prior scrolling to an index.
  ...
  */updateSizeForIndex: function (index) {
  ...
  _debounce: function (name, cb, asyncModule) {
    if (IS_V2) {
      ...
      enqueueDebouncer(this.debounce(name, cb));
    }
  },
  ...
  /* Templatizer bindings for v2 */_forwardHostPropV2: function (prop, value) {
  ...
  /* Templatizer bindings for v1 */_getStampedChildren: function () {
  ...
  /* Gets the activeElement of the shadow root/host that contains the list. */_getActiveElement: function () {
...
$_documentContainer$2.innerHTML = `<dom-module id="paper-material-styles">
  <template>
    <style>
      :host, html {
        --paper-material: {
...
$_documentContainer$5.innerHTML = `<dom-module id="paper-icon-button">
...
$_documentContainer$7.innerHTML = `<iron-iconset-svg name="dylan-icons" size="24">
  <svg>
    <defs>
      <g id="menu">
...
document.head.appendChild($_documentContainer$7.content); // Gesture events like tap and track generated from touch will not be
...
class DylanApp extends PolymerElement {
  ...
  static get observers() {
    return ['_routePageChanged(_routeData.page)'];
  }
  ...
  _pageChanged(page) {
    switch (page) {
      case 'home':
        import('./dylan-home.js').then(bundle => bundle && bundle.$dylanHome || {});
        break;

      case 'all':
        import('./dylan-all.js').then(bundle => bundle && bundle.$dylanAll || {});
        break;

      case 'graphics':
        import('./dylan-graphics.js').then(bundle => bundle && bundle.$dylanGraphics || {});
        break;

      case 'personal':
        import('./dylan-personal.js').then(bundle => bundle && bundle.$dylanPersonal || {});
        break;

      case 'developer':
        import('./dylan-developer.js').then(bundle => bundle && bundle.$dylanDeveloper || {});
        break;

      case 'designer':
        import('./dylan-designer.js').then(bundle => bundle && bundle.$dylanDesigner || {});
        break;

      case 'social':
        import('./dylan-social.js').then(bundle => bundle && bundle.$dylanSocial || {});
        break;

      case 'contact':
        import('./dylan-contact.js').then(bundle => bundle && bundle.$dylanContact || {});
        break;

      case '404':
        import('./dylan-404.js').then(bundle => bundle && bundle.$dylan$404 || {});
        break;
    }
  }
...
$_documentContainer$8.innerHTML = `<dom-module id="shared-styles">
  <template>
    <style>
