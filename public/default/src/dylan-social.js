...
class DylanSocial extends PolymerElement {
  static get template() {
    return html`
            <style>
                :host {
            ...
            <iron-ajax
                auto
                handle-as="json"
                url="[[rootPath]]data/elements_social.json"
                last-response="{{elementsSocial}}"></iron-ajax>

            <iron-list items="[[elementsSocial]]" as="item" grid>
                <template>
                    <div>
                        <element-card
...
window.customElements.define('dylan-social', DylanSocial);